#!/usr/bin/env python
"""Generate the Common Voice French (fr) regression dataset (refs + Whisper-tiny hyps).
...
*all* splits — the French train is many GB across multiple shards), we fetch just the test
...
language forced to French. Audio is never committed — only the resulting text.
...
exact stack (that lock is the provenance record). Run inside it:
    cd regression/datasets/common_voice_fr/generate
