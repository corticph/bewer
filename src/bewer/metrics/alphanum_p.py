...
class AlphaNumP(Metric):
    ...
    description = (
        ...
        "alphanumerical entities. Entities are detected via a regex predicate applied to each token (and to "
        "hyphen-joined runs of consecutive tokens) using the token's case-preserving form. The default "
        "pattern matches a candidate string when (a) it contains at least one uppercase letter and is not "
        "an ordinary capitalised compound (one or more parts that are each either init-cap or all "
        "lowercase, joined by hyphens — so 'Patient', 'Hello', 'Hello-World', 'up-to-date' are excluded "
        "while 'MRI', 'mmHg', 'CT-scan', 'X-ray', 'pre-MRI', 'MRI-CT' are kept), (b) it consists of one or "
        "more letters followed by at least one digit, or (c) it contains at least one Greek letter (so μg, "
        "α-helix, β-blocker are entities regardless of case). Hyphen structure is enforced strictly: a "
        "multi-token compound entity is TP only when both ref and hyp preserve the hyphens; a hyp compound "
        "that invents a hyphen not in ref counts as FP. The pattern uses Unicode letter properties "
        "(\\p{Lu}, \\p{Ll}, \\p{L}, \\p{Greek}), so Greek letters and other scripts integrate naturally. "
        "This metric is inherently case-sensitive — ASR systems that emit lowercase-only output will lose "
        "case-only entities (e.g. 'mri' vs 'MRI' counts as a miss). Period-separated tokens (e.g. 'Dr.', "
        "'e.g.') are not detected because the default tokenizer splits on periods."
    )
