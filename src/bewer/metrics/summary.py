...
class DatasetSummary(Metric):
    ...
    def num_ref_words(self) -> int:
        ...
        return sum(self._get_example_metric(example).num_ref_words for example in self._src)

    @metric_value
    def num_ref_chars(self) -> int:
        """The total number of characters in the reference texts."""
        return sum(self._get_example_metric(example).num_ref_chars for example in self._src)

    @metric_value
    def num_hyp_words(self) -> int:
        """The total number of tokens in the hypothesis texts."""
        return sum(self._get_example_metric(example).num_hyp_words for example in self._src)

    @metric_value
    def num_hyp_chars(self) -> int:
        """The total number of characters in the hypothesis texts."""
        return sum(self._get_example_metric(example).num_hyp_chars for example in self._src)
