...
def _resolve_tokenizer(name, cfg):
...
def resolve_pipelines(cfg: OmegaConf) -> dict[str, dict[str, callable]]:
    ...
    for name, pipeline_cfg in cfg.standardizers.items():
        pipelines.standardizers[name] = _resolve_func_pipeline(name, pipeline_cfg)
    for name, pipeline_cfg in cfg.tokenizers.items():
        pipelines.tokenizers[name] = _resolve_tokenizer(name, pipeline_cfg)
    for name, pipeline_cfg in cfg.normalizers.items():
        pipelines.normalizers[name] = _resolve_func_pipeline(name, pipeline_cfg)
