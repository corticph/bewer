"""Tests for bewer.metrics.insertion_run_rate module."""

import pytest

from bewer.metrics.insertion_run_rate import InsertionRunRate, InsertionRunRate_


@pytest.fixture
def dataset_insertions():
    """Dataset with various insertion patterns."""
    dataset = __import__("bewer").Dataset()
    dataset.add("hello world", "hello world")  # perfect match
    dataset.add("hello world", "hello there world")  # 1 insertion (run of 1)
    dataset.add("hello world", "hello foo bar world")  # 2 insertions (run of 2)
    dataset.add("a b c d", "a x y b z c d")  # runs [2, 1]
    return dataset


@pytest.fixture
def dataset_all_insertions():
    """Dataset where hyp is pure insertion (empty ref)."""
    dataset = __import__("bewer").Dataset()
    dataset.add("", "hello world")
    return dataset


class TestInsertionRunRateExampleMetric:
    """Tests for InsertionRunRate_ (ExampleMetric) class."""

    def test_num_insertions_perfect_match(self, dataset_perfect_match):
        """Test num_insertions is 0 for perfect match."""
        em = dataset_perfect_match[0].metrics.insertion_run_rate()
        assert em.num_insertions == 0

    def test_num_insertions_single_insertion(self, dataset_insertions):
        """Test num_insertions counts a single insertion."""
        em = dataset_insertions[1].metrics.insertion_run_rate()
        assert em.num_insertions == 1

    def test_num_insertions_contiguous_run(self, dataset_insertions):
        """Test num_insertions counts a contiguous run of 2."""
        em = dataset_insertions[2].metrics.insertion_run_rate()
        assert em.num_insertions == 2

    def test_num_insertions_multiple_runs(self, dataset_insertions):
        """Test num_insertions counts across multiple runs."""
        em = dataset_insertions[3].metrics.insertion_run_rate()
        assert em.num_insertions == 3

    def test_num_ops_perfect_match(self, dataset_perfect_match):
        """Test num_ops for perfect match (2 matches, 0 edits)."""
        em = dataset_perfect_match[0].metrics.insertion_run_rate()
        assert em.num_ops == 2

    def test_num_ops_with_insertions(self, dataset_insertions):
        """Test num_ops includes matches + edits."""
        em = dataset_insertions[1].metrics.insertion_run_rate()
        # MATCH("hello"), INSERT("there"), MATCH("world") = 3 ops
        assert em.num_ops == 3

    def test_max_run_length_perfect_match(self, dataset_perfect_match):
        """Test max_run_length is 0 for perfect match."""
        em = dataset_perfect_match[0].metrics.insertion_run_rate()
        assert em.max_run_length == 0

    def test_max_run_length_single(self, dataset_insertions):
        """Test max_run_length for single insertion."""
        em = dataset_insertions[1].metrics.insertion_run_rate()
        assert em.max_run_length == 1

    def test_max_run_length_contiguous(self, dataset_insertions):
        """Test max_run_length for contiguous run."""
        em = dataset_insertions[2].metrics.insertion_run_rate()
        assert em.max_run_length == 2

    def test_max_run_length_multiple_runs(self, dataset_insertions):
        """Test max_run_length picks the longest run."""
        em = dataset_insertions[3].metrics.insertion_run_rate()
        # runs [2, 1] → max = 2
        assert em.max_run_length == 2

    def test_value_perfect_match(self, dataset_perfect_match):
        """Test value is 0.0 for perfect match."""
        em = dataset_perfect_match[0].metrics.insertion_run_rate()
        assert em.value == 0.0

    def test_value_single_insertion_default(self, dataset_insertions):
        """Test value with min_run_length=1 (default) counts all insertions."""
        em = dataset_insertions[1].metrics.insertion_run_rate()
        # 1 insertion / 3 ops = 1/3
        assert em.value == pytest.approx(1 / 3)

    def test_value_single_insertion_min_run_2(self, dataset_insertions):
        """Test value with min_run_length=2 excludes single insertions."""
        em = dataset_insertions[1].metrics.insertion_run_rate(min_run_length=2)
        # 0 qualifying insertions / 3 ops = 0.0
        assert em.value == 0.0

    def test_value_contiguous_run_default(self, dataset_insertions):
        """Test value with min_run_length=1 counts all insertions."""
        em = dataset_insertions[2].metrics.insertion_run_rate()
        # 2 insertions / 4 ops = 0.5
        assert em.value == 0.5

    def test_value_contiguous_run_min_run_2(self, dataset_insertions):
        """Test value with min_run_length=2 includes run of 2."""
        em = dataset_insertions[2].metrics.insertion_run_rate(min_run_length=2)
        # 2 qualifying / 4 ops = 0.5
        assert em.value == 0.5

    def test_value_contiguous_run_min_run_3(self, dataset_insertions):
        """Test value with min_run_length=3 excludes run of 2."""
        em = dataset_insertions[2].metrics.insertion_run_rate(min_run_length=3)
        assert em.value == 0.0

    def test_value_multiple_runs_default(self, dataset_insertions):
        """Test value with min_run_length=1 counts all insertions."""
        em = dataset_insertions[3].metrics.insertion_run_rate()
        # 3 insertions / 7 ops = 3/7
        assert em.value == pytest.approx(3 / 7)

    def test_value_multiple_runs_min_run_2(self, dataset_insertions):
        """Test value with min_run_length=2 counts only run of 2."""
        em = dataset_insertions[3].metrics.insertion_run_rate(min_run_length=2)
        # 2 qualifying / 7 ops = 2/7
        assert em.value == pytest.approx(2 / 7)

    def test_value_all_insertions(self, dataset_all_insertions):
        """Test value when hyp is pure insertion."""
        em = dataset_all_insertions[0].metrics.insertion_run_rate()
        # 2 insertions / 2 ops = 1.0
        assert em.value == 1.0


class TestInsertionRunRateEmptyReference:
    """Tests for IRR edge case: empty reference and hypothesis."""

    def test_empty_ref_and_hyp(self, empty_dataset):
        """Test that empty ref and hyp gives 0.0."""
        empty_dataset.add("", "")
        em = empty_dataset[0].metrics.insertion_run_rate()
        assert em.num_ops == 0
        assert em.value == 0.0


class TestInsertionRunRateDatasetMetric:
    """Tests for InsertionRunRate (dataset-level Metric) class."""

    def test_num_insertions_aggregates(self, dataset_insertions):
        """Test that dataset num_insertions aggregates example values."""
        irr = dataset_insertions.metrics.insertion_run_rate()
        expected = sum(ex.metrics.insertion_run_rate().num_insertions for ex in dataset_insertions)
        assert irr.num_insertions == expected

    def test_num_run_insertions_default(self, dataset_insertions):
        """Test num_run_insertions with min_run_length=1 equals num_insertions."""
        irr = dataset_insertions.metrics.insertion_run_rate()
        assert irr.num_run_insertions == irr.num_insertions

    def test_num_run_insertions_min_run_2(self, dataset_insertions):
        """Test num_run_insertions with min_run_length=2 excludes singletons."""
        irr = dataset_insertions.metrics.insertion_run_rate(min_run_length=2)
        # Only runs >= 2: example 2 (run of 2) + example 3 (run of 2) = 2 + 2 = 4
        assert irr.num_run_insertions == 4

    def test_num_ops_aggregates(self, dataset_insertions):
        """Test that dataset num_ops aggregates example values."""
        irr = dataset_insertions.metrics.insertion_run_rate()
        expected = sum(ex.metrics.insertion_run_rate().num_ops for ex in dataset_insertions)
        assert irr.num_ops == expected

    def test_value_calculation(self, dataset_insertions):
        """Test dataset-level IRR value calculation."""
        irr = dataset_insertions.metrics.insertion_run_rate()
        assert irr.value == irr.num_run_insertions / irr.num_ops

    def test_value_perfect_match_dataset(self, dataset_perfect_match):
        """Test IRR is 0 for dataset with all perfect matches."""
        irr = dataset_perfect_match.metrics.insertion_run_rate()
        assert irr.value == 0.0

    def test_value_min_run_2(self, dataset_insertions):
        """Test dataset-level value with min_run_length=2."""
        irr = dataset_insertions.metrics.insertion_run_rate(min_run_length=2)
        # num_run_insertions=4, num_ops=2+3+4+7=16
        assert irr.value == pytest.approx(4 / 16)

    def test_empty_dataset(self, empty_dataset):
        """Test IRR on empty dataset."""
        irr = empty_dataset.metrics.insertion_run_rate()
        assert irr.num_ops == 0
        assert irr.num_insertions == 0
        assert irr.value == 0.0


class TestInsertionRunRateMetricAttributes:
    """Tests for InsertionRunRate metric attributes."""

    def test_short_name(self, sample_dataset):
        """Test IRR short_name."""
        irr = sample_dataset.metrics.insertion_run_rate()
        assert irr.short_name_base == "IRR"

    def test_long_name(self, sample_dataset):
        """Test IRR long_name."""
        irr = sample_dataset.metrics.insertion_run_rate()
        assert irr.long_name_base == "Insertion Run Rate"

    def test_description(self, sample_dataset):
        """Test IRR has description."""
        irr = sample_dataset.metrics.insertion_run_rate()
        assert len(irr.description) > 0

    def test_example_cls(self, sample_dataset):
        """Test IRR has example_cls set."""
        irr = sample_dataset.metrics.insertion_run_rate()
        assert irr.example_cls == InsertionRunRate_

    def test_short_name_includes_params(self, sample_dataset):
        """Test short_name includes parameters."""
        irr = sample_dataset.metrics.insertion_run_rate(min_run_length=3)
        assert "min_run_length=3" in irr.short_name

    def test_long_name_includes_params(self, sample_dataset):
        """Test long_name includes parameters."""
        irr = sample_dataset.metrics.insertion_run_rate(min_run_length=3)
        assert "min_run_length=3" in irr.long_name


class TestInsertionRunRateMetricValues:
    """Tests for InsertionRunRate metric_values method."""

    def test_metric_values_main(self):
        """Test that value is the main metric."""
        values = InsertionRunRate.metric_values()
        assert values["main"] == "value"

    def test_metric_values_other(self):
        """Test that other metric values are present."""
        values = InsertionRunRate.metric_values()
        assert "num_insertions" in values["other"]
        assert "num_run_insertions" in values["other"]
        assert "num_ops" in values["other"]

    def test_example_metric_values(self):
        """Test InsertionRunRate_ metric_values."""
        values = InsertionRunRate_.metric_values()
        assert values["main"] == "value"
        assert "num_insertions" in values["other"]
        assert "num_run_insertions" in values["other"]
        assert "num_ops" in values["other"]
        assert "max_run_length" in values["other"]


class TestInsertionRunRateValidation:
    """Tests for InsertionRunRate parameter validation."""

    def test_min_run_length_zero_raises(self, sample_dataset):
        """Test that min_run_length=0 raises ValueError."""
        with pytest.raises(ValueError, match="min_run_length must be >= 1"):
            sample_dataset.metrics.insertion_run_rate(min_run_length=0)

    def test_min_run_length_negative_raises(self, sample_dataset):
        """Test that negative min_run_length raises ValueError."""
        with pytest.raises(ValueError, match="min_run_length must be >= 1"):
            sample_dataset.metrics.insertion_run_rate(min_run_length=-1)
