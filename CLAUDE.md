...
- Insertion rate: `insertion_rate.py` — counts insertions that are part of contiguous insertion runs of length >= `run_length` (default 1), normalized by the number of reference tokens. When `run_length=1`, all insertions are counted. When `run_length>=2`, only burst insertion runs are counted — a hallucination signal. Depends on the Levenshtein metric for alignment.
